...
*/
	...
	if (!$nntp->connect()) {
		...
		$group_info = $nntp->join_group($group_name);

		if ($group_info == NULL) {
			echo "<b>".$messages_ini["error"]["group_fail"].$group_name." </b><br>";
			echo $nntp->get_error_message()."<br>";
			exit;
		} else {
			...
			$article_list = $nntp->get_article_list($group_name, false);
...
<rss version="2.0">
	<channel>
		<title><? echo $group_name; ?></title>
<?
		echo "<description>".$messages_ini["text"]["newsgroup"]." ".$group_name;
		$desc = $nntp->get_groups_description($group_name);
		if (sizeof($desc) > 0) {
			echo " - ".$desc[$group_name];
		}
		echo "</description>";
		// Quit sooner to release resources
		$nntp->quit();
?>
		<link><? echo construct_url("newsgroups.php?group=".urlencode($group_name)); ?></link>
<?
		echo "<lastBuildDate>".date("D, d M Y H:i:s O", time())."</lastBuildDate>";
?>
		<generator>Web-News v.1.6.2 (NNTP to RSS Engine)</generator> 
		<image>
			...
			<width>40</width>
			<height>40</height>
			<description>RSS feed provided by Web-News v.1.6.2 (NNTP to RSS Engine)</description>
		</image>
<?
	foreach ($message_summary as $mid=>$message) {
		...
		$link = htmlescape(construct_url("newsgroups.php?art_group=".urlencode($group_name)."&article_id=$mid"));
		echo "<link>";
		echo $link;
		echo "</link>\n";
