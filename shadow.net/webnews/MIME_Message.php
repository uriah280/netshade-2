...
//					$parts = preg_split("/^begin\s+[0-9][0-9][0-9]\s+(.+?)\s*\r?\n/m", $body, -1, PREG_SPLIT_DELIM_CAPTURE);
					...
					{ // FOR NOW, assume uuencode if no encoding is specified
					    $header["content-transfer-encoding"] = "uuencode";
					}
					...
					for ($i = 1;$i < sizeof($parts);$i += 3) {
						$this->MIME_message[] = array(
													"header"=>array("content-type"=>'unknown', //get_content_type($parts[$i]),
